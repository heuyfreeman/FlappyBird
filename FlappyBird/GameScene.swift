...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var scrollNode:SKNode! //地面、雲、壁の親ノード
    var wallNode:SKNode!   //
    ...
    var score = 0
    var itemScore = 0   //##
    var scoreLabelNode:SKLabelNode!  //スコア表示
    ...
    func setupWall(){
        ...
        /* 下の壁のY軸下限位置(中央位置から下方向の最大振れ幅で下の壁を表示する位置)を計算
         positionでは、nodeの中央位置を座標で指定するので、代入するための値を変数に入れる
         */
        let groundSize = SKTexture(imageNamed: "ground").size()
        //地面上部、地上から上の中央の高さ
        let center_y = groundSize.height + (self.frame.size.height - groundSize.height) / 2
        /* wallは400pxある。半分の200が中央位置なので、200マイナスする。
         すり抜け幅120/2 , 上下幅120/2
         */
        ...
        let createWallAnimation = SKAction.run({
            ...
            //アイテム用ノード
            let itemRandom_y = CGFloat.random(in: -(random_y_range / 5)..<random_y_range / 5)
            let grapesTexture = SKTexture(imageNamed: "grapes")
            grapesTexture.filteringMode = .linear
            let itemSprite = SKSpriteNode(texture: grapesTexture)
            itemSprite.position = CGPoint( x: upper.size.width * 2.5, y: wallTexture.size().height / 2 + under_wall_y + slit_length / 2 + itemRandom_y)
            //物理演算
            itemSprite.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: itemSprite.size.width,height: itemSprite.size.height))
            //itemSprite.physicsBody = SKPhysicsBody(texture: itemSprite.texture!, size: itemSprite.texture!.size())
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        /* And演算 両方のbitが１なら1、この場合scoreCategoryは1なので、
            AかBのどちらかがscoreCategoryなら、1で真となる
        */
